...
function(e, t) {
    ...
    function a(e) {
        function t(e) {
            var t = d("#langSelect span.select-text").text().indexOf("检测到") > -1,
            n = d.trim(d("#langSelect span.select-text").text()).replace("检测到：", "").replace(/\s+/g, "");
            if (n.indexOf("»") < 0) {
    ...
    function s(e) {
        if (0 == d('li[data-value="' + e + '"]').length) d("#langSelect").selectValue("AUTO");
            ...
            L.state.isSelectLan || d(".select-text").text("检测到：" + t)
        }
    }
    function l(e) {
    ...
    t.translate = function(e, t) {
        ...
        if (R(), _.text(i), i > 5e3) {
            ...
            d("#inputTargetError").text("有道翻译字数限制为5000字，“" + s + "”及其后面没有被翻译!").show(),
...
function(e) {
    ...
    return {
        ...
        isWeb: function(e) {
            return /\b((?:[a-z][\w\-]+:(?:\/{1,3}|[a-z0-9%])|www\d{0,3}[.]|[a-z0-9.\-]+[.][a-z]{2,4}\/)(?:[^\s()<>]|\((?:[^\s()<>]|(?:\([^\s()<>]+\)))*\))+(?:\((?:[^\s()<>]|(?:\([^\s()<>]+\)))*\)|[^\s`!()\[\]{};:'".,<>?«»“”‘’]))/i.test(e.toLocaleString())
        },
        addToFav: function() {
